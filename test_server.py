...
try:
    ...
    # /process returns status, reply, plus evaluator-required intelligence keys
    required_keys = {"status", "reply", "scamDetected", "intelligenceFlags",
                     "extractedIntelligence", "engagementMetrics", "agentNotes",
                     "totalMessagesExchanged"}
    check("process returns required keys", required_keys.issubset(set(result.keys())),
...
try:
    ...
    # /process returns intelligence keys alongside status+reply
    check("process with artifacts returns scamDetected", "scamDetected" in result)
except Exception as e:
    failed += 1
